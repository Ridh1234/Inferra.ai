...
function runOAuthFlow(callback) {
...
function sendToBackend() {
    chrome.storage.local.get(["userName", "transcript", "chatMessages", "meetingTitle", "meetingStartTimeStamp", "meetingEndTimeStamp", "attendees", "speakers","oauthEmail", "oauthName", "screenshots"], function (result) {
        ...
        if (result.userName && result.transcript && result.chatMessages) {
            ...
            // Send meeting data to Node.js backend as before
            ...
            // Send transcript data to AI-backend for summarization
            ...
            .then(response => {
                if (response.ok) {
                    ...
                    throw new Error('Failed to get report from AI-backend');
                }
            })
            .then(blob => {
                // Optionally, save or display the PDF report
                const url = window.URL.createObjectURL(blob);
                chrome.downloads.download({ url, filename: 'meeting_report.pdf' });
            })
            .catch((error) => {
                console.error('Error fetching report from AI-backend:', error);
            });

            // Placeholder: Send audio file to AI-backend for transcription
            // You need to capture audio and send as FormData
            // Example:
