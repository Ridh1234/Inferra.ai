...
const extensionStatusJSON_bug = {
  "status": 400,
  "message": "<strong>Inferra encountered a new error</strong> <br /> Please report it <a href='https://github.com/sikehish/inferra/issues' target='_blank'>here</a>."
}
const reportErrorMessage = "There is a bug in Inferra. Please report it at https://github.com/sikehish/inferra/issues"
...
async function checkExtensionStatus() {
  ...
  chrome.storage.local.set({
    extensionStatusJSON: { status: 200, message: "<strong>Inferra is running</strong> <br /> Do not turn off captions" },
  });
  ...
  await fetch(
    ...
    .then((result) => {
      ...
      result.message = "<strong>Inferra is running</strong> <br /> Do not turn off captions";
...
function meetingRoutines(uiType) {
  ...
  checkElement(meetingEndIconData.selector, meetingEndIconData.text).then(() => {
    ...
    try {
      ...
      // Move observers to higher scope
      ...
      const transcriptTargetNode = document.querySelector('.a4cQT')
        ...
        if (transcriptTargetNode && transcriptTargetNode.firstChild) {
          try {
          ...
          console.warn('Transcript target node or its firstChild not found. Skipping observer setup.');
        }
      ...
      setTimeout(() => {
        ...
        // CRITICAL DOM DEPENDENCY. Grab the chat messages element. This element is present, irrespective of chat ON/OFF, once it appears for this first time.
          ...
          if (chatMessagesTargetNode) {
            ...
            console.warn('Chat messages target node not found. Skipping observer setup.');
          }
      }, 500)
      ...
      chrome.storage.sync.get(["operationMode"], function (result) {
        if (result.operationMode == "manual")
          showNotification({ status: 400, message: "<strong>Inferra is not running</strong> <br /> Turn on captions using the CC icon, if needed" })
        else
          showNotification(extensionStatusJSON)
      })
...
function showNotification(extensionStatusJSON) { 
    ...
    // Inferra text next to the logo
    logoText.style.cssText = logoTextCSS;
    logoText.innerHTML = "Inferra";
...
function transcriber(mutationsList, observer) {
  try {
    ...
    mutationsList.forEach(mutation => {
      ...
      const a4cQTNode = document.querySelector('.a4cQT');
      let people = [];
      if (a4cQTNode && a4cQTNode.firstChild && a4cQTNode.firstChild.firstChild && a4cQTNode.firstChild.firstChild.childNodes) {
        people = a4cQTNode.firstChild.firstChild.childNodes;
        if (people.length > 0) {
          // Get the last person
  ...
  try {
    mutationsList.forEach(mutation => {
      ...
      if (chatMessagesElement && chatMessagesElement.children.length > 0) {
        ...
        let personName = '';
        if (chatMessageElement && chatMessageElement.firstChild && chatMessageElement.firstChild.firstChild) {
          personName = chatMessageElement.firstChild.firstChild.textContent;
        } else {
          console.warn('Chat message element or its children not found.');
        }
        ...
        let chatMessageText = '';
        if (chatMessageElement && chatMessageElement.lastChild && chatMessageElement.lastChild.lastChild) {
          chatMessageText = chatMessageElement.lastChild.lastChild.textContent;
        }
